...
const RequestedPermits: React.FC = () => {
  ...
  const getStatusBadge = (status: string, currentDepartmentIndex: number, template: any) => {
    if (status === 'approved') return <Badge className="bg-green-500">Approved</Badge>;
    if (status === 'rejected') return <Badge variant="destructive">Rejected</Badge>;

    const flow = template?.approvalFlow || [];
    if (currentDepartmentIndex < flow.length) {
      return <Badge variant="secondary">Pending at {flow[currentDepartmentIndex]?.name || 'Department'}</Badge>;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-bold">Requested Permits</h2>
      </div>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    {getStatusBadge(permit.status, permit.currentDepartmentIndex, permit.templateId)}
                  </div>
        ...
        {permits.length === 0 && (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-muted-foreground mb-4">You haven't submitted any permits yet.</p>
              <Button onClick={() => setActiveView('create-permit')}>
