...
const RejectedPermits: React.FC = () => {
  ...
  const fetchPermits = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const rejected = data.permits.filter((p: Permit) => p.status === 'rejected');
        setPermits(rejected);
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-bold">Rejected Permits</h2>
      </div>
        ...
        {permits.length === 0 && (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">No rejected permits</h3>
              <p className="text-muted-foreground">You don't have any rejected permits.</p>
            </CardContent>
          </Card>
        )}
      </div>
